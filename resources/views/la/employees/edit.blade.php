	...
	<div class="box-body">
		<div class="row">
			<div class="col-md-8 col-md-offset-2">
				{!! Form::model($employee, ['route' => [config('laraadmin.adminRoute') . '.employees.update', $employee->id ], 'method'=>'PUT', 'id' => 'employee-edit-form', 'enctype'=>'multipart/form-data']) !!}
					...
					<div class="form-group">
						...
						<select class="form-control select2-hidden-accessible" data-placeholder="Enter Country" rel="select2" name="country" tabindex="-1" aria-hidden="true">
							@foreach($countriesList as $code => $name)
								<option value="{{$name}}" {{$employee->country == $name? 'selected': ''}}>{{$name}}</option>
							@endforeach
						</select>
					</div>
					<div class="form-group">
						<label for="zipcode">Zipcode :</label>
						<input class="form-control" placeholder="Enter Zipcode" data-rule-maxlength="50" name="zipcode" type="text" value="{{$employee->zipcode}}" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" maxlength="10" minlength="6">
					</div>
					...
					{!! LAFormMaker::input($module, 'salary_cur','','','form-control',['oninput'=>"this.value = this.value.replace(/[^0-9+]/g, '').replace(/(\..*)\./g, '$1');", 'minlength'=>'0', 'maxlength'=>'2']) !!}
				...
				<div class="form-group {{ $errors->has('profile_picture') ? ' has-error' : '' }}">
					<div>
						<img src= "{{ asset('storage/images/'.$user->profile_picture) }}" width="50%" height="auto">
					</div>
