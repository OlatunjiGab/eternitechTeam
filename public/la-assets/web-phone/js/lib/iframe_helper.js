...
function SendMessageToFrame(msg)
{
    ...
    // queue messages until web-phone is loaded into the frame
    ...
    {
        PutToDebugLog(2, logtag + 'EVENT, SendMessageToFrame web-phone not loaded yet, queue message: ' + msg);
        mesage_queue_cache.push(msg);

        //start timer for checking web-phone frame window availability
        StopQueueTimer();
    ...
    function HandleMessageParent(event)
    {
        try{ try{
            ...
            if (typeof(event.data) !== 'undefined' && event.data !== null && event.data.toString().indexOf('_IFRAME') >= 0)
            {
                ...
                {
                    PutToDebugLog(2, logtag + 'EVENT, onMessage: received message: web-phone is loaded and ready');
