 ...
 * "loaded" - the web-phone library is completely loaded so you can start the usage (call any other API functions after this state is received).
 ...
 * "registered" - the web-phone is registered to the VoIP server, so you can start dialing/receiving calls/chats
 * "unregistered" - the web-phone is unregistered from the VoIP server
 * "failed" - the web-phone failed to register to the VoIP server
 * --PARAMETERS --
...
* In some circumstances the initialization procedure might take a few seconds (depending on usable engines) so you can prepare the web-phone with this method
* to avoid any delay when the user really needs to use by pressing the call button for example. 
* If the serveraddress/username/password is already set and auto register is not disabled, then the web-phone will also register (connect) to the SIP server upon start.
* If start is not called, then the web-phone will initialize itself the first time when you call some other function such as register or call.
* The web-phone parameter should be set before you call this method (preset in the js file or by using the setparameter function).
*/
function start ()
{
 ...
 *                  or ERROR, if web-phone is not yet started
*/
function getavailablecallfunc ()
{
    if (typeof (webphone_api.plhandler) === 'undefined' || webphone_api.plhandler === null)
    {
        ...
        return 'ERROR, web-phone is not yet started';
    }
    else
    {
...
* and the web-phone will streamline them properly. Use the space character to insert delays between the digits.
...
/**
* Create new contact in the web-phone's phonebook/contact list.
* --PARAMETERS --
...
/**
* Delete existing contact from web-phone's phonebook/contact list, if matches the passed name and/or number.
* --PARAMETERS --
...
function setparameter (param, value)
{
    ...
    if (typeof (webphone_api.plhandler) === 'undefined' || webphone_api.plhandler === null)
    {
        webphone_api.Log('WARNING, web-phone not initialized yet. webphone_api.setparameter AddToQueue: ' + param + ': ' + value);
...
/** Will set the current line. (Use only if you present line selection for the users. Otherwise, you don’t have to take care about the lines as it is handled automatically by the web-phone). */
...
/** Will return the current active line number. This should be the line, which you have set previously except after incoming and outgoing calls (the web-phone will automatically switch the active line to a new free line for these if the current active line is already occupied by a call). */
...
/** Return true if the web-phone is registered ("connected") to the SIP server.*/
...
/** Change the color theme of the web-phone skin. It is the same as from Settings -> Theme
 * --PARAMETERS --
...
/** Call this function passing a callback.
 * The passed callback function will be called with one parameter, which will be web-phone's work directory.
 ...
 *  !!! IMPORTANT: This API call will stop any runing engine. You will need to call start() to start the web-phone */
...
/** Returns a string containing all the past logs of the web-phone*/
function getlogs()
{
    if (typeof (webphone_api.plhandler) !== 'undefined' && webphone_api.plhandler !== null)
