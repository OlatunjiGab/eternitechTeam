...
class PermissionsController extends Controller
{
	...
	public function create()
	{
        ...
        if(Module::hasAccess($module->id,'create')) {
            return View('la.permissions.create', ['module' => $module]);
        } else {
            return redirect(config('laraadmin.adminRoute')."/");
        }
	}
	...
	public function dtAjax()
	{
		...
		for($i=0; $i < count($data->data); $i++) {
			...
			if($this->show_action) {
				...
				if(Module::hasAccess("Permissions", "delete")) {
					$output .= Form::open(['route' => [config('laraadmin.adminRoute') . '.permissions.destroy', $data->data[$i][0]], 'method' => 'delete', 'style'=>'display:inline', 'onsubmit'=>$this->deleteMessage]);
