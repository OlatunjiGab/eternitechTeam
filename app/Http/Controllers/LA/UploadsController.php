...
class UploadsController extends Controller
{
    ...
    public function get_file($hash, $name)
    {
        ...
        if(!isset($upload->id) || $upload->name != $name) {
            //return response()->json(SystemReponse::build(false, array('message' => "Unauthorized Access 1")));
        }
        ...
        if(!$upload->public && !isset(Auth::user()->id)) {
            return response()->json(SystemResponse::build(false, array('message' => "Unauthorized Access 2")));
        }
        ...
        if($upload->public) {
            ...
            return response()->json(SystemResponse::build(false, array('message' => "Unauthorized Access 3")));
        }
    }
			...
			if(Input::hasFile('file')) {
				...
				*/
				...
				if( $upload_success ) {
                    ...
                    return response()->json(SystemResponse::build(true, array('upload' => $upload)), 200);
				} else {
                    return response()->json(SystemResponse::build(false), 400);
				}
			} else {
                return response()->json(SystemResponse::build(false, "error: upload file not found."), 400);
			}
		} else {
			return response()->json(SystemResponse::build(false, array('message' => "Unauthorized Access")));
		}
    }
        ...
        if(Module::hasAccess("Uploads", "edit")) {
			...
			if(isset($upload->id)) {
				if($upload->user_id == Auth::user()->id || Entrust::hasRole('SUPER_ADMIN')) {
                    ...
                    return response()->json(SystemResponse::build(true));
	
				} else {
                    return response()->json(SystemResponse::build(false, array('message' => "Upload not found")));
				}
			} else {
                return response()->json(SystemResponse::build(false, array('message' => "Upload not found")));
			}
        ...
        if(Module::hasAccess("Uploads", "edit")) {
			...
			if(isset($upload->id)) {
				if($upload->user_id == Auth::user()->id || Entrust::hasRole('SUPER_ADMIN')) {
                    ...
                    return response()->json(SystemResponse::build(false, array('message' => "Unauthorized Access 1")));
				}
			} else {
                return response()->json(SystemResponse::build(false, array('message' => "Upload not found")));
			}
