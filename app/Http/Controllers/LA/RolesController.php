...
class RolesController extends Controller
{
	...
	public function create()
	{
        ...
        if(Module::hasAccess($module->id,'create')) {
            return View('la.roles.create', ['module' => $module]);
        } else {
            return redirect(config('laraadmin.adminRoute')."/");
        }
	}
	...
	public function dtAjax()
	{
		...
		for($i=0; $i < count($data->data); $i++) {
			...
			if($this->show_action) {
				...
				if(Module::hasAccess("Roles", "delete")) {
					$output .= Form::open(['route' => [config('laraadmin.adminRoute') . '.roles.destroy', $data->data[$i][0]], 'method' => 'delete', 'style'=>'display:inline', 'onsubmit'=>$this->deleteMessage]);
	...
	public function saveModuleRolePermissions(Request $request, $id)
	{
