...
class BackupsController extends Controller
{
		...
		if(Module::hasAccess("Backups", "create")) {
			...
			if(LAHelper::getLineWithString2($outputStr, "Copying ") == -1) {
				if(LAHelper::getLineWithString2($outputStr, "mysqldump: No such file or directory") != -1) {
                    return response()->json(SystemResponse::build(false, array('message' => "Configure dump_command_path in config/database.php. Check console for error details.",
                                                                             'exitCode' => $exitCode,
                                                                             'output' => $outputStr)));
				}
                return response()->json(SystemResponse::build(false, array('message' => "Error while creating Backup.",
                                                                                             'exitCode' => $exitCode,
                                                                                             'output' => $outputStr)));
			} else {
                ...
                return response()->json(SystemResponse::build(true, array(
            ...
            return response()->json(SystemResponse::build(false, array('message' => "No rights to create Backup.")));
		}
	}
	...
	public function dtAjax()
	{
		...
		for($i=0; $i < count($data->data); $i++) {
			...
			if($this->show_action) {
				...
				if(Module::hasAccess("Backups", "delete")) {
					$output .= Form::open(['route' => [config('laraadmin.adminRoute') . '.backups.destroy', $data->data[$i][0]], 'method' => 'delete', 'style'=>'display:inline', 'onsubmit'=>$this->deleteMessage]);
	...
	public function downloadBackup($id) {
		...
		if(Module::hasAccess($module->id)) {
            ...
            return response()->json(SystemResponse::build(false, array('message' => "No rights to download Backup.")));
		}
	}
}
